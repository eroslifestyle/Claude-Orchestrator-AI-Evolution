...
exports.ResourceConstraintRecovery = void 0;
...
class SystemResourceMonitor {
    ...
    maxHistorySize = 100;
    constructor(config) {
        this.logger = new logger_1.PluginLogger('SystemResourceMonitor');
        this.config = config;
    }
    /**
    ...
    startMonitoring() {
        ...
        this.monitoringInterval = setInterval(() => this.collectResourceMetrics(), this.config.monitoringInterval);
    }
    /**
    ...
    async getCurrentMetrics() {
        ...
        try {
            ...
            const metrics = {
                ...
                uptime: os.uptime(),
                collectTime: perf_hooks_1.performance.now() - startTime
            };
            return metrics;
        }
        catch (error) {
     ...
     */
    predictResourceConstraints(lookAheadMinutes = 5) {
        const predictions = [];
        ...
        if (memoryTrend > 0.05) { // 5% increase trend
            const timeToLimit = this.calculateTimeToThreshold(recent.map(r => r.memory.usage), 0.9, lookAheadMinutes);
        ...
        if (cpuTrend > 0.1) { // 10% increase trend
            const timeToLimit = this.calculateTimeToThreshold(recent.map(r => r.cpu.usage), 0.85, lookAheadMinutes);
            if (timeToLimit > 0 && timeToLimit <= lookAheadMinutes) {
                predictions.push({
                    type: 'cpu',
        ...
        if (diskTrend > 0.02) { // 2% increase trend
            const timeToLimit = this.calculateTimeToThreshold(recent.map(r => r.disk.usage), 0.95, lookAheadMinutes);
            if (timeToLimit > 0 && timeToLimit <= lookAheadMinutes) {
                predictions.push({
                    type: 'disk',
    ...
    checkImmediateConstraints(metrics) {
    ...
    calculateMemoryPressure(processMemory, systemMemory) {
        ...
        if (processUsage > 0.9 || systemUsage > 0.9)
            return 'critical';
        if (processUsage > 0.8 || systemUsage > 0.8)
            return 'high';
        if (processUsage > 0.7 || systemUsage > 0.7)
            return 'medium';
        return 'low';
    }
    ...
    calculateTimeToThreshold(values, threshold, maxMinutes) {
        if (values.length < 2)
            return -1;
        const trend = this.calculateTrend(values);
        if (trend <= 0)
            return -1; // No increasing trend
...
class ResourceCleanupManager {
    ...
    tempDirectories = [];
    constructor() {
        this.logger = new logger_1.PluginLogger('ResourceCleanupManager');
        this.initializeTempDirectories();
    }
    /**
     * Perform comprehensive resource cleanup
     */
    async performResourceCleanup(strategy) {
        const startTime = perf_hooks_1.performance.now();
        ...
        try {
            ...
            cleanupResult.cleanupTime = perf_hooks_1.performance.now() - startTime;
     ...
     */
    async performMemoryCleanup(strategy) {
            ...
            switch (strategy) {
                ...
                case 'aggressive':
                    ...
                    if (require.cache) {
                        ...
                        const nonEssentialModules = Object.keys(require.cache).filter(path => path.includes('node_modules') && !path.includes('core'));
     ...
     */
    async performDiskCleanup(strategy) {
        let filesRemoved = 0;
        let spaceFreed = 0;
        try {
    ...
    async cleanupDirectory(dirPath, strategy) {
        let filesRemoved = 0;
        let spaceFreed = 0;
        try {
    ...
    initializeTempDirectories() {
        this.tempDirectories.push(os.tmpdir(), path.join(process.cwd(), 'temp'), path.join(process.cwd(), 'tmp'), path.join(process.cwd(), '.tmp'));
    }
}
/**
...
class LoadDistributionManager {
    ...
    constructor(maxConcurrent = 5) {
        this.logger = new logger_1.PluginLogger('LoadDistributionManager');
        this.maxConcurrentOperations = maxConcurrent;
    }
    /**
     * Execute operation with load balancing
     */
    async executeWithLoadBalancing(operationId, operation, priority = 1) {
    ...
    setOperationInactive(operationId) {
 ...
 */
class ResourceConstraintRecovery extends events_1.EventEmitter {
    logger;
    resourceMonitor;
    cleanupManager;
    loadManager;
    ...
    constructor(config) {
        super();
        this.logger = new logger_1.PluginLogger('ResourceConstraintRecovery');
        this.config = {
     ...
     */
    async handleResourceConstraint(context) {
        ...
        try {
            ...
            recoveryResult.success = this.verifyRecoverySuccess(context, recoveryResult.resourcesBefore, recoveryResult.resourcesAfter);
            // Step 5: Additional recovery attempts if needed
            if (!recoveryResult.success && this.config.recovery.maxRecoveryAttempts > 1) {
                const additionalAttempts = await this.performAdditionalRecoveryAttempts(context, recoveryResult.resourcesAfter, recoveryId);
                recoveryResult.strategiesApplied.push(...additionalAttempts);
                // Final verification
                recoveryResult.resourcesAfter = await this.resourceMonitor.getCurrentMetrics();
                recoveryResult.success = this.verifyRecoverySuccess(context, recoveryResult.resourcesBefore, recoveryResult.resourcesAfter);
            }
            recoveryResult.recoveryTime = perf_hooks_1.performance.now() - startTime;
            ...
            const failedResult = {
                ...
                resourcesBefore: await this.resourceMonitor.getCurrentMetrics(),
                resourcesAfter: null,
                recoveryTime: perf_hooks_1.performance.now() - startTime,
                error: error.message,
                timestamp: new Date().toISOString()
            };
     ...
     */
    determineRecoveryStrategy(context, metrics) {
    ...
    determineMemoryRecoveryStrategy(severity, metrics) {
        const actions = [];
    ...
    determineCpuRecoveryStrategy(severity, metrics) {
        const actions = [];
    ...
    determineDiskRecoveryStrategy(severity, metrics) {
        const actions = [];
    ...
    determineTimeoutRecoveryStrategy(severity, metrics) {
        return {
            type: 'timeout',
     ...
     */
    async executeRecoveryStrategy(strategy, context, recoveryId) {
        const startTime = perf_hooks_1.performance.now();
        try {
            const results = [];
     ...
     */
    async executeRecoveryAction(action, context, recoveryId) {
    ...
    applyEmergencyThrottling(context) {
     ...
     */
    async performAdditionalRecoveryAttempts(context, currentMetrics, recoveryId) {
        const attempts = [];
     ...
     */
    verifyRecoverySuccess(context, beforeMetrics, afterMetrics) {
     ...
     */
    async executeWithResourceProtection(operationId, operation, priority = 1) {
...
exports.ResourceConstraintRecovery = ResourceConstraintRecovery;
/**
 * Export Resource Constraint Recovery Engine
 */
exports.default = ResourceConstraintRecovery;
//# sourceMappingURL=ResourceConstraintRecovery.js.map