...
exports.createCommonCriteria = exports.validateSuccessCriteria = exports.createRalphLoopIntegration = exports.RalphLoopIntegration = void 0;
...
class RalphLoopIntegration {
    ...
    constructor(config, loopConfig) {
        this.logger = new logger_1.PluginLogger('RalphLoopIntegration');
     ...
     */
    async detectLoopRequirement(taskDescription, extractedKeywords) {
        ...
        try {
            ...
            const confidence = this.calculateDetectionConfidence(patternMatches, keywordScore, detectedCriteria);
            // 5. Decision logic
            const shouldUseLoop = confidence >= 0.7 && detectedCriteria.length > 0;
            // 6. Stima parametri ottimali
            const estimatedParams = this.estimateLoopParameters(taskDescription, detectedCriteria, confidence);
            const result = {
                shouldUseLoop,
                confidence,
                detectedCriteria,
                ...
                reasoning: this.generateDetectionReasoning(patternMatches, keywordScore, detectedCriteria, confidence)
            };
     ...
     */
    async executeRalphLoop(initialPrompt, successCriteria, options) {
        const startTime = performance.now();
        ...
        try {
            ...
            for (let i = 1; i <= config.maxIterations; i++) {
                ...
                // Execute iteration
                const iteration = await this.executeIteration(initialPrompt, context, successCriteria, config);
    ...
    analyzeKeywordsForLoop(keywords) {
        ...
        const relevantKeywords = keywords.filter(kw => iterativeKeywords.some(iterKw => kw.text.toLowerCase().includes(iterKw) ||
    ...
    calculateDetectionConfidence(patternMatches, keywordScore, criteria) {
        // Weight factors
    ...
    estimateLoopParameters(text, criteria, confidence) {
        // Base estimates
    ...
    generateDetectionReasoning(patternMatches, keywordScore, criteria, confidence) {
        const reasons = [];
        if (confidence >= 0.8) {
    ...
    async executeIteration(prompt, context, criteria, config) {
        ...
        const progressScore = this.calculateIterationProgress(simulatedOutput, context, criteria);
    ...
    calculateIterationProgress(output, context, criteria) {
        // Calculate progress based on multiple factors
        let progress = 0.0;
        // Factor 1: Criteria satisfaction
        const satisfiedCount = criteria.filter(criterion => this.evaluateSingleCriterion(criterion, output, context)).length;
    ...
    evaluateSuccessCriteria(criteria, iteration) {
        // All required criteria must be satisfied
        const requiredCriteria = criteria.filter(c => c.required);
        const requiredSatisfied = requiredCriteria.every(criterion => iteration.satisfiedCriteria.includes(criterion.id));
    ...
    evaluateSingleCriterion(criterion, output, context) {
    ...
    detectStagnation(context, config) {
    ...
    calculateLoopMetrics(context, totalTime) {
...
exports.RalphLoopIntegration = RalphLoopIntegration;
// =============================================================================
// FACTORY FUNCTION
// =============================================================================
/**
 ...
 */
function createRalphLoopIntegration(config, loopConfig) {
    return new RalphLoopIntegration(config, loopConfig);
}
exports.createRalphLoopIntegration = createRalphLoopIntegration;
// =============================================================================
// UTILITY FUNCTIONS
// =============================================================================
/**
 ...
 */
function createCommonCriteria(taskType) {
    switch (taskType) {
        case 'tdd':
            return [
                {
...
exports.createCommonCriteria = createCommonCriteria;
//# sourceMappingURL=RalphLoopIntegration.js.map