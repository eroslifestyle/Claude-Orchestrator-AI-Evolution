...
exports.applyParallelExecutionRule = exports.createParallelExecutionRule = exports.ParallelExecutionRule = void 0;
const events_1 = require("events");
// =============================================================================
// PARALLEL EXECUTION RULE ENGINE
// =============================================================================
class ParallelExecutionRule extends events_1.EventEmitter {
    config;
    tasks = new Map();
    executionPlan = null;
    metrics;
     ...
     */
    buildParallelExecutionPlan(tasks) {
        ...
        for (const level of sortedLevels) {
            ...
            const maxConcurrency = Math.min(taskIds.length, this.config.maxConcurrentAgents);
            // Estimate duration (max of all tasks since parallel)
            const estimatedDurationMs = Math.max(...tasksInLevel.map(t => t.estimatedDurationMs));
        ...
        const estimatedTotalTimeMs = batches.reduce((sum, b) => sum + b.estimatedDurationMs, 0);
        const estimatedSequentialTimeMs = tasks.reduce((sum, t) => sum + t.estimatedDurationMs, 0);
     ...
     */
    calculateCriticalPath(tasks, levels) {
        const taskMap = new Map(tasks.map(t => [t.id, t]));
     ...
     */
    async executeWithMaxParallelism(taskExecutor) {
        ...
        for (const batch of this.executionPlan.batches) {
            ...
            for (const blockedBy of batch.blockedBy) {
                ...
                if (blockingBatch) {
                    const incomplete = blockingBatch.taskIds.filter(id => !this.completedTasks.has(id) && !this.failedTasks.has(id));
            ...
            this.metrics.maxConcurrentReached = Math.max(this.metrics.maxConcurrentReached, this.runningTasks.size);
            ...
            const results = await Promise.allSettled(tasksToExecute.map(async (task) => {
                try {
                    const result = await taskExecutor(task);
                    return { taskId: task.id, ...result };
                }
                catch (error) {
                    return { taskId: task.id, success: false, error: error };
                }
            }));
            ...
            this.emit('batchCompleted', {
                ...
                completed: results.filter(r => r.status === 'fulfilled' && r.value.success).length,
    ...
    async waitForTasks(taskIds) {
        return new Promise((resolve) => {
            const checkInterval = setInterval(() => {
                const allDone = taskIds.every(id => this.completedTasks.has(id) || this.failedTasks.has(id));
                if (allDone) {
     ...
     */
    rebalanceOnFailure(failedTaskId) {
        if (!this.executionPlan)
            return [];
    ...
    getExecutionState() {
        ...
        return {
            ...
            pending: allTaskIds.filter(id => !this.runningTasks.has(id) &&
...
exports.ParallelExecutionRule = ParallelExecutionRule;
// =============================================================================
// FACTORY FUNCTION
// =============================================================================
function createParallelExecutionRule(config) {
    return new ParallelExecutionRule(config);
}
exports.createParallelExecutionRule = createParallelExecutionRule;
// =============================================================================
// INTEGRATION HELPER
// =============================================================================
/**
 ...
 */
function applyParallelExecutionRule(orchestrator, config) {
    const rule = createParallelExecutionRule(config);
    // Hook into orchestrator's task queue
    if (orchestrator.on) {
        orchestrator.on('tasksReady', (tasks) => {
...
exports.applyParallelExecutionRule = applyParallelExecutionRule;
//# sourceMappingURL=ParallelExecutionRule.js.map