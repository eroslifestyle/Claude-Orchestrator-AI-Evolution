...
exports.OrchestratorV60 = void 0;
// Enhanced imports
const analysis_1 = require("./analysis");
const task_launcher_1 = require("./execution/task-launcher");
...
class OrchestratorV60 {
    ...
    constructor() {
        ...
        this.taskLauncher = new task_launcher_1.TaskLauncher();
        this.progressTracker = new progress_tracker_1.ProgressTracker(`orchestrator-${Date.now()}`);
     ...
     */
    async analyzeTaskIntelligent(userRequest) {
     ...
     */
    fallbackToLegacyAnalysis(userRequest) {
        ...
        for (const [keyword, expertFile] of Object.entries(this.KEYWORD_TO_EXPERT_MAPPING)) {
            if (requestLower.includes(keyword)) {
                ...
                if (expertFile.includes('gui'))
                    ...
                    foundDomains.add('General');
            }
        }
        const dominioCount = foundDomains.size;
        const wordCount = userRequest.split(' ').length;
        let complessita;
        if (dominioCount <= 1 && wordCount <= 10)
            complessita = 'bassa';
        else if (dominioCount <= 2 && wordCount <= 20)
            complessita = 'media';
        else
            complessita = 'alta';
        return {
     ...
     */
    routeToAgentsIntelligent(analysis, userRequest) {
        console.log('\n🎯 STEP 2: ROUTING INTELLIGENTE basato su Analysis Engine');
        ...
        if (analysis.analysisResult.success) {
            const primaryTask = this.createEnhancedTaskFromDomain(analysis.analysisResult.domains.primaryDomain, userRequest, `T${taskCounter}`, analysis.analysisResult);
            if (primaryTask) {
            ...
            for (const secondaryDomain of analysis.analysisResult.domains.secondaryDomains) {
                if (secondaryDomain.confidence >= 0.6) { // Soglia per domini secondari
                    const secondaryTask = this.createEnhancedTaskFromDomain(secondaryDomain, userRequest, `T${taskCounter}`, analysis.analysisResult);
                    if (secondaryTask) {
     ...
     */
    createEnhancedTaskFromDomain(domain, // ClassifiedDomain type
    userRequest, taskId, analysisResult) {
        const expertFile = domain.suggestedAgent;
        if (!expertFile)
            return null;
        // Map priority da domain
        let priority = 'MEDIA';
        if (domain.priority === 'CRITICA')
            priority = 'CRITICA';
        else if (domain.priority === 'ALTA')
            priority = 'ALTA';
        else if (domain.priority === 'MEDIA')
            priority = 'MEDIA';
        else
            priority = 'BASSA';
        const task = {
     ...
     */
    createFallbackTask(userRequest, taskId, analysis) {
        return {
            id: taskId,
     ...
     */
    displayEnhancedExecutionPlan(tasks, analysis) {
        console.log('\n📋 STEP 3: COMUNICAZIONE PRE-LANCIO con INTELLIGENCE INSIGHTS');
     ...
     */
    async executeEnhanced(tasks, analysis) {
     ...
     */
    async executeParallelEnhanced(tasks) {
     ...
     */
    async executeEnhancedTaskLevel(tasks, level) {
     ...
     */
    async executeEnhancedAgent(task) {
     ...
     */
    generateEnhancedReport(tasks, analysis) {
     ...
     */
    async orchestrateEnhanced(userRequest) {
    ...
    // UTILITY METHODS
    // ============================================================================
    getSpecializationFromDomain(domain) {
        ...
        return specializations[domain] || 'General Development';
    }
    averageConfidence(tasks) {
        if (tasks.length === 0)
            return 0;
        return Math.round(tasks.reduce((sum, t) => sum + t.confidence, 0) / tasks.length * 100);
    }
    getSourceDistribution(tasks) {
...
exports.OrchestratorV60 = OrchestratorV60;
