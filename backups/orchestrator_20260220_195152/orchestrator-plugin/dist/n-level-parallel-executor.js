...
exports.NLevelTaskGraph = exports.CommunicationHub = exports.NLevelParallelExecutor = void 0;
...
exports.NLevelTaskGraph = NLevelTaskGraph;
// =============================================================================
// N-LEVEL PARALLEL EXECUTOR
// =============================================================================
class NLevelParallelExecutor extends events_1.EventEmitter {
    config;
    graph;
    ...
    async execute() {
        ...
        while (this.isExecuting && !this.graph.isComplete()) {
            ...
            const readyTasks = this.graph.getReadyTasks(this.config.maxConcurrent - this.runningTasks.size);
    ...
    async executeTaskAsync(task) {
        ...
        try {
            ...
            this.graph.markFailed(task.id, error.message);
            this.runningTasks.delete(task.id);
            ...
            console.log(`   ❌ [${task.path}] FAILED: ${error.message}`);
        }
    }
    /**
     ...
     */
    async spawnDynamicSubtasks(parent, subtaskConfigs) {
...
exports.NLevelParallelExecutor = NLevelParallelExecutor;
// =============================================================================
// COMMUNICATION HUB - Per comunicazione Agent → Orchestrator
// =============================================================================
class CommunicationHub extends events_1.EventEmitter {
    messageLog = [];
    /**
     * Agent invia risultato all'orchestrator
     */
    agentReport(taskId, report) {
     ...
     */
    requestSubtasks(taskId, subtasks) {
     ...
     */
    requestInfo(fromTaskId, targetTaskId, query) {
     ...
     */
    orchestratorCommand(taskId, command, payload) {
     ...
     */
    getMessageLog(count = 100) {
        return this.messageLog.slice(-count);
    }
    /**
     * Log interno
     */
    log(from, type, payload) {
...
*/
//# sourceMappingURL=n-level-parallel-executor.js.map