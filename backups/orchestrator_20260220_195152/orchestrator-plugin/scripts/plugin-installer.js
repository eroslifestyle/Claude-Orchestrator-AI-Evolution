...
exports.PluginInstallerCLI = exports.PluginInstaller = exports.PluginPackager = exports.PluginValidator = void 0;
...
class PluginValidator {
     ...
     */
    validateClaudeCompatibility(pluginPath) {
        const errors = [];
        const warnings = [];
        const suggestions = [];
        try {
...
class PluginPackager {
    ...
    async packagePlugin(pluginPath, outputPath) {
        const spinner = (0, ora_1.default)('Packaging plugin...').start();
        try {
    ...
    buildPluginMetadata(pluginPath) {
        const packageJsonPath = path.join(pluginPath, 'package.json');
        const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
    ...
    getPluginFiles(pluginPath) {
        const files = [];
        const distPath = path.join(pluginPath, 'dist');
        const addFiles = (dir, relativeTo) => {
    ...
    async createPackageStructure(pluginPath, outputPath, metadata) {
    ...
    async generateInstallationManifest(packagePath, metadata) {
        const manifest = {
            ...metadata,
    ...
    async createArchive(packagePath) {
        const archivePath = `${packagePath}.tar.gz`;
        try {
            // Use tar command if available, otherwise skip archive creation
            (0, child_process_1.execSync)(`tar -czf "${archivePath}" -C "${path.dirname(packagePath)}" "${path.basename(packagePath)}"`, {
                stdio: 'pipe'
            });
            return archivePath;
        }
            ...
            console.log(chalk_1.default.yellow('tar command not available, skipping archive creation'));
            return packagePath;
        }
    }
...
exports.PluginPackager = PluginPackager;
/**
 * Plugin Installer
 */
class PluginInstaller {
    ...
    async installPlugin(pluginPath, options = {}) {
        const spinner = (0, ora_1.default)('Installing plugin...').start();
        try {
    ...
    async detectClaudeInstallation() {
    ...
    async installPluginFiles(pluginPath, claudePath, options) {
    ...
    async installDependencies(installPath) {
        ...
        try {
            ...
            console.log(chalk_1.default.yellow('Warning: Some dependencies may not have been installed'));
        }
    }
    async registerPlugin(installPath, claudePath) {
        ...
        if (fs.existsSync(pluginsConfigPath)) {
            try {
                ...
                console.log(chalk_1.default.yellow('Warning: Could not parse existing plugins.json, creating new one'));
            }
        }
        // Remove existing orchestrator plugin entry
        pluginsConfig.plugins = pluginsConfig.plugins.filter((p) => p.name !== 'orchestrator-plugin');
    ...
    async initializePlugin(installPath) {
    ...
    displaySuccessMessage(installPath) {
        console.log(chalk_1.default.bold.green('\n🎉 Installation Complete!\n'));
        const tableData = [
            [chalk_1.default.bold('Plugin'), 'Orchestrator Plugin v1.0.0'],
            [chalk_1.default.bold('Status'), chalk_1.default.green('✓ Installed Successfully')],
            [chalk_1.default.bold('Location'), installPath],
            [chalk_1.default.bold('Commands'), '5 commands registered'],
            [chalk_1.default.bold('Configuration'), '~/.claude/orchestrator/']
        ];
        console.log((0, table_1.table)(tableData));
        console.log(chalk_1.default.cyan('\n📚 Getting Started:'));
        console.log('1. Restart Claude Code to load the plugin');
        console.log('2. Try: ' + chalk_1.default.yellow('/orchestrator "your first request"'));
        console.log('3. Get help: ' + chalk_1.default.yellow('/orchestrator --help'));
        console.log('4. Configure: Run setup wizard with ' + chalk_1.default.yellow('/orchestrator-setup'));
     ...
     */
    async uninstallPlugin(claudePath) {
        const spinner = (0, ora_1.default)('Uninstalling plugin...').start();
        try {
            ...
            if (fs.existsSync(userConfigDir)) {
                ...
                console.log(chalk_1.default.gray('Remove manually if no longer needed'));
            }
...
class PluginInstallerCLI {
    ...
    async handleInstallCommand(args) {
        ...
        console.log(chalk_1.default.bold.cyan('🚀 Installing Orchestrator Plugin\n'));
    ...
    async handleUninstallCommand(args) {
        ...
        const claudePath = claudePathIndex >= 0 && claudePathIndex + 1 < args.length
            ...
            : undefined;
        console.log(chalk_1.default.bold.red('🗑️  Uninstalling Orchestrator Plugin\n'));
        await this.installer.uninstallPlugin(claudePath);
    }
    async handleValidateCommand(args) {
        const pluginPath = args[0] || process.cwd();
        console.log(chalk_1.default.bold.blue('🔍 Validating Plugin\n'));
        ...
        if (validation.valid && compatibility.valid) {
            console.log(chalk_1.default.green('✅ Plugin is valid'));
        }
        else {
            console.log(chalk_1.default.red('❌ Plugin validation failed'));
        }
        ...
        if (compatibility.errors.length > 0) {
            ...
            compatibility.errors.forEach(error => console.log(chalk_1.default.red(`  • ${error}`)));
        }
        const allWarnings = [...validation.warnings, ...compatibility.warnings];
        if (allWarnings.length > 0) {
            console.log(chalk_1.default.yellow('\nWarnings:'));
            allWarnings.forEach(warning => console.log(chalk_1.default.yellow(`  • ${warning}`)));
        }
        const allSuggestions = [...validation.suggestions, ...compatibility.suggestions];
        if (allSuggestions.length > 0) {
            console.log(chalk_1.default.cyan('\nSuggestions:'));
            allSuggestions.forEach(suggestion => console.log(chalk_1.default.cyan(`  • ${suggestion}`)));
        }
    }
    displayHelp() {
...
exports.PluginInstallerCLI = PluginInstallerCLI;
