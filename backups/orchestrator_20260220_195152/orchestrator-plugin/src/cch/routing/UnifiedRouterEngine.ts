...
// =============================================================================
// LRU MAP IMPLEMENTATION (Custom O(1) LRU)
// =============================================================================

/**
 * Entry nella cache LRU
 */
 ...
 */
class LRUMap<K extends string | number, V> {
  private capacity: number;
  private ttl: number;
  private cache: Map<K, LRUCacheEntry<V>>;
  private head: LRUCacheEntry<V> | null;
  private tail: LRUCacheEntry<V> | null;
  ...
  put(key: K, value: V): void {
    ...
    // Create new entry
    const newEntry: LRUCacheEntry<V> = {
      value,
      key: key as string,
      prev: null,
      next: null,
  ...
  private removeEntry(entry: LRUCacheEntry<V>): void {
    if (entry.prev) {
...
class SimplifiedTFIDF {
  ...
  private corpusFrequency: Map<string, number>;
   ...
   */
  extractKeywords(text: string, topN: number = 10): ExtractedKeyword[] {
    const tokens = this.tokenize(text);
    const termFrequency = new Map<string, number>();
 ...
 */
export class UnifiedRouterEngine {
  private logger: PluginLogger;
  private config: Required<UnifiedRouterConfig>;
  private cache: LRUMap<string, RoutingDecision>;
  private tfidf: SimplifiedTFIDF;
  private agentRegistry: Map<string, AgentRegistryEntry>;
  private stats: RouterStats;

  // Metrics collection
  private requestTimes: number[] = [];
  private readonly MAX_TIME_SAMPLES = 1000;

  constructor(config: UnifiedRouterConfig = {}) {
    this.logger = new PluginLogger('UnifiedRouterEngine');
   ...
   */
  async warmup(requests: TaskRequest[]): Promise<void> {
   ...
   */
  private computeRoutingDecision(request: TaskRequest, startTime: number): RoutingDecision {
    ...
    const fallbackAgents = this.buildFallbackChain(
      primaryAgentName,
      sortedAgents.slice(1).map(([name]) => name)
    );
  ...
  private scoreAgents(
    ...
    const complexityOrder: ComplexityLevel[] = ['low', 'medium', 'high', 'extreme'];
  ...
  private selectModel(
    ...
    requestComplexity?: ComplexityLevel
  ): Exclude<ModelType, 'auto'> {
    // Start with agent's default model, converting 'auto' to a concrete model
    let selectedModel: Exclude<ModelType, 'auto'> = agent.defaultModel === 'auto' ? 'sonnet' : agent.defaultModel;

    const complexity = requestComplexity || 'medium';

    // Upgrade based on complexity
    const modelHierarchy: Exclude<ModelType, 'auto'>[] = ['haiku', 'sonnet', 'opus'];
  ...
  private buildFallbackChain(
    ...
    const tiers: Array<'expert' | 'core' | 'generic'> =
      primary.tier === 'expert' ? ['core', 'generic'] :
   ...
   */
  private createFallbackDecision(request: TaskRequest, startTime: number): RoutingDecision {
    const fallbackAgent = this.agentRegistry.get('fallback') ||
   ...
   */
  private estimateComplexity(request: TaskRequest): ComplexityLevel {
 ...
 */
export function createUnifiedRouterEngine(
