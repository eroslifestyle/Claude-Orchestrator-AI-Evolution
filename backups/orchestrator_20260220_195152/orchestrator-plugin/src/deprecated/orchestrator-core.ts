...
import { OrchestratorQuickFixer } from './fixes/orchestrator-quick-fixes';
...
interface AgentTask {
  ...
  // 🚀 PARALLELISMO A 3 LIVELLI - ARCHITETTURA GERARCHICA
  level: 1 | 2 | 3;                    // Livello orchestrazione (1=principal, 2=sub, 3=micro)
  parentTaskId?: string;               // ID del parent task (per livelli 2-3)
  subTasks?: AgentTask[];              // Sub-tasks generati da questo agent
  allowSubSpawning?: boolean;          // Se può creare sub-agents
  complexityThreshold?: number;        // Soglia complessità per spawning (0-1)
  maxSubTasks?: number;                // Limite max sub-tasks per controllo costi
  spawnRules?: SubTaskSpawnRule[];     // Regole per generazione intelligente

  // Additional properties for hierarchical task tracking
  depth?: number;                      // Depth in the task tree (0 = root)
  childTaskIds?: string[];             // IDs of child subtasks
  maxDepth?: number;                   // Maximum depth for spawning
  path?: string;                       // Path in the tree: "T1.2.3.1"
  rootTaskId?: string;                 // ID of the root L1 task
}

interface SubTaskSpawnRule {
  triggerKeywords: string[];           // Keywords che attivano lo spawning
  targetExpertFile: string;           // Expert file da utilizzare per sub-task
  maxComplexity: number;              // Complessità max per questo tipo di sub-task
  description: string;                // Descrizione del tipo di sub-task
}
...
interface ExecutionPlan {
  ...
  totalAgents: number;
  stimatedTime: string;
  documenterTask: AgentTask; // REGOLA #5: SEMPRE presente come ultimo

  // 🚀 PARALLELISMO A 3 LIVELLI
  hierarchicalStructure: {
    level1Tasks: AgentTask[];         // Task principali
    level2Tasks: AgentTask[];         // Sub-tasks
    level3Tasks: AgentTask[];         // Micro-tasks
    totalLevels: number;              // Profondità massima raggiunta
    maxParallelism: number;           // Max agent contemporanei possibili
  };
}
...
class OrchestratorV51 {
  private quickFixer: OrchestratorQuickFixer;

  constructor() {
    this.quickFixer = new OrchestratorQuickFixer();
  }
  ...
  analyzeTask(userRequest: string): KeywordAnalysis {
    ...
    for (const [keyword, expertFile] of Object.entries(KEYWORD_TO_EXPERT_MAPPING)) {
      ...
      if (keywordRegex.test(requestLower)) {
        ...
        else if (expertFile.includes('mql')) foundDomains.add('MQL');
        else if (expertFile.includes('trading')) foundDomains.add('Trading');
        else if (expertFile.includes('architect')) foundDomains.add('Architecture');
        else if (expertFile.includes('tester')) foundDomains.add('Testing');
        else if (expertFile.includes('devops')) foundDomains.add('DevOps');
        else if (expertFile.includes('languages')) foundDomains.add('Languages');
        else if (expertFile.includes('mobile')) foundDomains.add('Mobile');
        else if (expertFile.includes('social')) foundDomains.add('Social');
        else if (expertFile.includes('core')) foundDomains.add('Core');
      }
    }
    ...
    let complessita: 'bassa' | 'media' | 'alta';
    // FIX #4: <5 agents = bassa, 5-10 = media, >10 = alta
    if (keywordCount < 3 && dominioCount <= 1) complessita = 'bassa';
    else if (keywordCount <= 6 && dominioCount <= 3) complessita = 'media';
  ...
  routeToAgents(analysis: KeywordAnalysis, userRequest: string): AgentTask[] {
    ...
    for (const keyword of analysis.keywords) {
      const expertFile = (KEYWORD_TO_EXPERT_MAPPING as any)[keyword];

      if (expertFile && !usedExperts.has(expertFile)) {
        usedExperts.add(expertFile);

        const model = (EXPERT_TO_MODEL_MAPPING as any)[expertFile] || 'sonnet';
        const priority = (EXPERT_TO_PRIORITY_MAPPING as any)[expertFile] || 'MEDIA';
        ...
        const task: AgentTask = {
          ...
          dependencies: [],
          priority: priority as 'CRITICA' | 'ALTA' | 'MEDIA',
  ...
  // Helper methods
  private generateTaskDescription(keyword: string, userRequest: string): string {
    const descriptions = {
  ...
  private getSpecialization(expertFile: string): string {
  ...
  private analyzeTaskComplexity(task: AgentTask): number {
    ...
    if (description.includes('e ') || description.includes(' + ')) complexity += 0.2;

    // Domini specifici = più complessità
    if (task.agentExpertFile.includes('gui')) complexity += 0.3;
    if (task.agentExpertFile.includes('database')) complexity += 0.2;
    if (task.agentExpertFile.includes('security')) complexity += 0.4; // Security = sempre complesso
    if (task.agentExpertFile.includes('architect')) complexity += 0.3;

    // Parole chiave complesse
    if (description.includes('integra') || description.includes('connett')) complexity += 0.2;
    if (description.includes('sicur') || description.includes('auth')) complexity += 0.3;
    if (description.includes('ottimizz') || description.includes('performance')) complexity += 0.2;
   ...
   */
  private generateSubTasks(parentTask: AgentTask, startCounter: number, level: number = 2): AgentTask[] {
   ...
   */
  private displayHierarchy(allTasks: AgentTask[]): void {
   ...
   */
  async executeParallel(tasks: AgentTask[]): Promise<void> {
    console.log('\n⚡ STEP 4: ESECUZIONE PARALLELA A 3 LIVELLI');
   ...
   */
  private async executeTaskLevel(tasks: AgentTask[], level: number): Promise<void> {
  ...
  private estimateTimeHierarchical(tasks: AgentTask[]): string {
    const parallelTime = this.calculateParallelTime(tasks);
    return `${parallelTime.toFixed(1)}-${(parallelTime + 1).toFixed(1)} minuti`;
  }

  private estimateCostHierarchical(tasks: AgentTask[]): string {
    return this.estimateCost(tasks); // Riusa metodo esistente
  }

  private calculateSequentialTime(tasks: AgentTask[]): number {
    // Tempo se eseguiti tutti in sequenza
    return tasks.length * 2.5; // 2.5 min average per task
  }

  private calculateParallelTime(tasks: AgentTask[]): number {
  ...
  private estimateCostForLevel(tasks: AgentTask[]): string {
  ...
  private estimateTime(tasks: AgentTask[]): string {
    const workTasks = tasks.filter(t => !t.agentExpertFile.includes('documenter'));
  ...
  private estimateCost(tasks: AgentTask[]): string {
    const costMap = { 'haiku': 0.02, 'sonnet': 0.08, 'opus': 0.25 };
    let totalCost = 0;
