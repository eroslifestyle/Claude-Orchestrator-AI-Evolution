...
export class SmartPathAnalyzer {
  private config: SmartPathInternalConfig;
  private confidenceScorer: ConfidenceScorer;
  private keywordCache: KeywordExtractionCache;
  private synonymMappings: Map<string, SynonymMapping[]>;
  private phrasePatterns: PhrasePattern[];
  private contextRules: ContextRule[];
  private metrics: SmartPathMetrics;
   ...
   */
  private async extractKeywordsSmart(text: string): Promise<ExtractedKeyword[]> {
   ...
   */
  private extractSynonymMatches(text: string, _tokens: string[]): ExtractedKeyword[] {
    const keywords: ExtractedKeyword[] = [];
    ...
    for (const [domain, mappings] of this.synonymMappings.entries()) {
      for (const mapping of mappings) {
        ...
        if (baseMatch) {
          keywords.push(this.createKeywordFromMatch(
            baseMatch,
            mapping.baseKeyword,
            domain,
            'exact',
        ...
        for (const synonym of mapping.synonyms) {
          ...
          if (synonymMatch) {
            keywords.push(this.createKeywordFromMatch(
              synonymMatch,
              mapping.baseKeyword, // Use base keyword as canonical form
              domain,
              'synonym',
              'direct',
              text
   ...
   */
  private extractPhraseMatches(text: string, _tokens: string[]): ExtractedKeyword[] {
    const keywords: ExtractedKeyword[] = [];
    const textLower = text.toLowerCase();
  ...
  private extractContextMatches(
    text: string,
    _tokens: string[],
    _existingKeywords: ExtractedKeyword[]
  ): ExtractedKeyword[] {
    const keywords: ExtractedKeyword[] = [];
    const textLower = text.toLowerCase();
    ...
    for (const rule of this.contextRules) {
      ...
      const contextWindow = this.extractContextWindow(
        text,
        triggerMatch.position,
        this.config.contextWindow
      );

      const hasAmplifiers = rule.amplifiers.some(amp =>
        contextWindow.toLowerCase().includes(amp)
      );

      const hasDampeners = rule.dampeners.some(damp =>
        contextWindow.toLowerCase().includes(damp)
      );
  ...
  private extractFuzzyMatches(
    ...
    const allBaseKeywords = new Set<string>();
    ...
    for (const token of tokens) {
      if (token.length < 3 || existingTexts.has(token.toLowerCase())) continue;
        ...
        if (distance <= this.config.fuzzyDistanceMax && distance > 0) {
          ...
          if (match) {
            ...
            if (domain) {
              keywords.push(this.createKeywordFromMatch(
                match,
                baseKeyword, // Use base keyword come canonical
                domain,
                'fuzzy',
                'partial',
                text
   ...
   */
  private findWordInText(text: string, word: string): { position: number; length: number } | null {
   ...
   */
  private findPhraseInText(text: string, pattern: PhrasePattern): Array<{ position: number; length: number }> {
   ...
   */
  private extractContextWindow(text: string, position: number, windowSize: number): string {
   ...
   */
  private findDomainForKeyword(keyword: string): string | null {
    for (const [domain, mappings] of this.synonymMappings.entries()) {
  ...
  private createKeywordFromMatch(
    match: { position: number; length: number },
    ...
    const confidenceContext = createConfidenceContext(tempKeyword, originalText, 'smart', {
      domainCount: 1,
   ...
   */
  private deduplicateAndScore(keywords: ExtractedKeyword[], _text: string): ExtractedKeyword[] {
  ...
  private getQuickConfidenceScore(text: string): number {
    ...
    for (const [, mappings] of this.synonymMappings.entries()) {
      for (const mapping of mappings) {
        ...
        const hasSynonym = mapping.synonyms.some(syn =>
          tokens.some(token => token === syn)
        );
   ...
   */
  private loadSynonymMappings(): void {
    ...
    for (const [domain, domainMappings] of Object.entries(mappings)) {
      this.synonymMappings.set(domain, domainMappings as SynonymMapping[]);
    }
   ...
   */
  private loadPhrasePatterns(): void {
   ...
   * Update metrics
   */
  private updateMetrics(cacheStats: CacheStats, responseTime: number): void {
