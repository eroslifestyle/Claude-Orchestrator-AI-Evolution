...
export interface AgentRegistry {
    ...
    keywordIndex: Map<string, string[]>;  // keyword -> agent ids
    categoryIndex: Map<string, string[]>; // category -> agent ids
    lastScan: number;
}
...
export class AgentDiscovery extends EventEmitter {
    ...
    private cacheValidMs: number = 300000; // 5 minuti cache

    // Mapping keywords -> expert files
    private static KEYWORD_AGENT_MAP: Record<string, string> = {
        // GUI & Frontend
    ...
    private static FALLBACK_AGENTS: Record<string, string> = {
    ...
    private parseAgentFile(
        ...
        let preferredModel: 'haiku' | 'sonnet' | 'opus' = 'sonnet';
        ...
        return {
            ...
            filePath,
            relativePath,
            category: category as AgentInfo['category'],
            keywords,
            description,
            preferredModel,
            priority,
    ...
    private async scanPlugins(): Promise<void> {
        ...
        if (!fs.existsSync(pluginCachePath)) return;
    ...
    findAgentForTask(taskDescription: string): DiscoveryResult {
        ...
        const matchedAgents: Map<string, number> = new Map(); // agentPath -> score
            ...
            if (agent) {
                return {
                    ...
                    alternatives: sortedMatches.slice(1, 4)
                        ...
                        .filter(a => a !== undefined) as AgentInfo[],
                    suggestions: [],
                    missingPlugins: []
                };
            }
     ...
     */
    private findAlternatives(originalPath: string): DiscoveryResult {
        const alternatives: AgentInfo[] = [];
        const suggestions: string[] = [];
        const missingPlugins: PluginInfo[] = [];
...
`;
        ...
        const categories: AgentInfo['category'][] = ['core', 'expert', 'workflow', 'template', 'system', 'docs'];
