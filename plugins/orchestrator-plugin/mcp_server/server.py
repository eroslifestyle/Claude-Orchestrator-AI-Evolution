...
# =============================================================================
# RE-EXPORTS - Import from separate modules for backward compatibility
# =============================================================================

# Version exports
from mcp_server.version import get_version, __skill_version__, __version__, __mcp_version__, __api_version__

# Activation exports
from mcp_server.activation import TaskComplexity, detect_task_complexity, should_use_orchestrator, get_execution_mode

# Model selector exports
from mcp_server.model_selector import ModelType, IntelligentModelSelector, get_model_selector

# Context tier exports
from mcp_server.context_tiers import get_context_tier, build_context_injection, ContextTier

# Context scorer exports
from mcp_server.context_scorer import is_context_sufficient, get_clarifying_questions

# Auto promotion exports
from mcp_server.auto_promotion import AutoPromoter, get_auto_promoter

# Session manager exports
from mcp_server.session_manager import SessionManager, get_session_manager, SessionStatus

# Session resume exports
from mcp_server.session_resume import get_resume_handler, check_and_prompt_resume

# Agent permissions exports
from mcp_server.agent_permissions import AgentPermissionManager as PermissionManager, get_permission_manager, inject_tool_permissions_into_agent_prompt

# =============================================================================
# FIX #4: CENTRALIZED KEYWORD LOADER - Load from JSON config
# =============================================================================

def load_keyword_mappings_from_json(file_path: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
    """
    FIX #4: Load keyword mappings from centralized JSON config file.
    Falls back to empty dict if file not found or invalid JSON.

    Args:
        file_path: Optional path to keyword mappings file.
                   If not provided, uses the default KEYWORD_MAPPINGS path.

    Returns:
        Dictionary containing keyword mappings, or empty dict on error.
    """
    target_path = file_path if file_path is not None else KEYWORD_MAPPINGS

    try:
        if os.path.exists(target_path):
            with open(target_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                logger.info(f"Loaded keyword mappings from {target_path}")
                return data
        else:
            logger.warning(f"Keyword mappings file not found: {target_path}")
    except (FileNotFoundError, IOError, OSError) as e:
        logger.error(f"IO error loading keyword mappings from {target_path}: {e}")
    except json.JSONDecodeError as e:
        logger.error(f"Invalid JSON in keyword mappings file {target_path}: {e}")
    except Exception as e:
        logger.error(f"Unexpected error loading keyword mappings: {e}")
    return {}
...
class OrchestratorEngine:
    ...
    _sessions: Dict[str, OrchestrationSession] = field(default_factory=dict)
    ...
    @property
    def sessions(self) -> Dict[str, OrchestrationSession]:
        """Get the sessions dictionary (public accessor for _sessions)."""
        return self._sessions

    @property
    def sessions_lock(self) -> threading.RLock:
        """Get the sessions lock (public accessor for _lock)."""
        return self._lock
    ...
    def model_selector(self) -> Any:
        ...
        if self._model_selector is None:
            try:
                ...
                # If model selector is not available, return None
    ...
    def _cleanup_old_sessions(self) -> int:
        ...
        with self._lock:
            ...
            age_threshold = datetime.now() - timedelta(hours=SESSION_MAX_AGE_HOURS)

            # Find sessions to remove (old sessions, or excess sessions)
            for session_id, session in self.sessions.items():
                # Remove old completed sessions
                if session.status == TaskStatus.COMPLETED and session.started_at < age_threshold:
                    to_remove.append(session_id)
                # Remove old failed sessions
                elif session.status == TaskStatus.FAILED and session.started_at < age_threshold:
                    to_remove.append(session_id)
                # Remove old pending sessions (very old pending sessions are likely abandoned)
                elif session.status == TaskStatus.PENDING and session.started_at < age_threshold:
                    to_remove.append(session_id)

            # If still over limit, remove oldest sessions (regardless of status)
            if len(self.sessions) - len(to_remove) > MAX_ACTIVE_SESSIONS:
                # Sort remaining sessions by started_at
                remaining_sessions = [
                    (sid, s) for sid, s in self.sessions.items()
                    if sid not in to_remove
                ]
                remaining_sessions.sort(key=lambda x: x[1].started_at)

                # Remove oldest until under limit
                excess = len(self.sessions) - len(to_remove) - MAX_ACTIVE_SESSIONS
                for i in range(excess):
                    sid = remaining_sessions[i][0]
                    # Prevent duplicates: check if not already in to_remove
                    if sid not in to_remove:
    ...
    def _check_and_cleanup_sessions(self) -> None:
        """
        Check if cleanup is needed based on session count and perform cleanup if necessary.
        Increments session counter and triggers cleanup when threshold is reached.
        """
        self._session_count_since_cleanup += 1

        # Trigger cleanup if we've reached the check interval
        if self._session_count_since_cleanup >= CLEANUP_CHECK_INTERVAL:
            removed = self._cleanup_old_sessions()
            self._session_count_since_cleanup = 0
            logger.debug(f"Session cleanup check: removed {removed} sessions")
    ...
    def generate_execution_plan(self, user_request: str) -> ExecutionPlan:
        ...
        # Check and cleanup sessions if needed
    ...
    def format_plan_table(self, plan: ExecutionPlan) -> str:
        ...
        lines.append(f"├─ Max concurrent agents: {max((len(b) for b in plan.parallel_batches), default=0)}")
    ...
    def generate_task_doc_template(self, task: AgentTask) -> str:
        ...
        specialization_section = f"\n### Specialization:\n{task.specialization}\n" if task.specialization else ""
        return f"""
## Task {task.id}: {task.description[:50]}{specialization_section}
### What was done:
...
"""
    ...
    def get_expert_model(self, expert_file: str, request_context: Optional[Dict[str, Any]] = None) -> str:
        """
        Get the appropriate model for a given expert file.

        Args:
            expert_file: Path to the expert file (e.g., 'core/coder.md')
            request_context: Optional context about the request for model selection

        Returns:
            Model name (e.g., 'opus', 'sonnet', 'haiku')
        """
        return EXPERT_TO_MODEL_MAPPING.get(expert_file, "sonnet")


# Global engine instance
engine = OrchestratorEngine()

# Global model selector instance (if available)
_model_selector: Optional[Any] = None


def get_model_selector() -> Optional[Any]:
    """
    Get the global model selector instance.
    Returns None if model selector is not available.
    """
...
def get_expert_model(expert_file: str, request_context: Optional[str] = None) -> str:
    """
    Get the appropriate model for a given expert file.

    This function uses the model selector if available, otherwise falls back
    to the direct mapping.
    ...
    if selector:
        return selector.get_model_for_agent_file(expert_file, request_context)
    return EXPERT_TO_MODEL_MAPPING.get(expert_file, "sonnet")
