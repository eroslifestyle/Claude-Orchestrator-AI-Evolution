...
import {
  ...
  TaskNode
} from './execution/ParallelExecutionRule';

import {
  AgentContextManager,
  createAgentContextManager,
  getGlobalContextManager
} from './execution/AgentContextManager';

import {
  OrchestratorVisualizer,
  createOrchestratorVisualizer,
  visualizer
} from './logging/OrchestratorVisualizer';

import {
  ErrorRecoveryManager,
  createErrorRecoveryManager,
  ModelTier
} from './execution/ErrorRecoveryManager';

import {
  TokenBudgetManager,
  createTokenBudgetManager
} from './execution/TokenBudgetManager';
...
interface StressTestConfig {
  ...
  taskDurationMs: { min: number; max: number };
...
function generateTestTasks(config: StressTestConfig): TaskNode[] {
  const tasks: TaskNode[] = [];
  const domains = ['GUI', 'Database', 'API', 'Security', 'Testing', 'Documentation'];
  const models: Array<'opus' | 'sonnet' | 'haiku'> = ['opus', 'sonnet', 'haiku'];
  const priorities: Array<'CRITICA' | 'ALTA' | 'MEDIA' | 'BASSA'> = ['CRITICA', 'ALTA', 'MEDIA', 'BASSA'];
...
async function executeTaskWithRetry(
  ...
  currentModel: ModelTier = 'sonnet'
): Promise<{ success: boolean; result?: any; error?: Error; attempts: number }> {
  ...
  if (prepResult.wasCleared && config.verbose) {
    visualizer.contextClear(
      task.id,
      prepResult.clearResult?.previousTokenCount || 0,
      'pre_execution'
    );
  }

  // REGOLA 3: Log inizio task
  visualizer.taskStart(task.id, task.agentType);
  ...
  try {
    ...
    const recoveryResult = await recoveryManager.handleError(
      task.id,
      task.agentType,
      error as Error,
      currentModel,
...
export async function runIntegratedStressTest(
  userConfig: Partial<StressTestConfig> = {}
): Promise<{
  success: boolean;
  metrics: any;
  errors: Error[];
...
`);
  ...
  const vis = createOrchestratorVisualizer({
    enabled: true,
    showTimestamps: true,
  ...
  const tokenBudget = createTokenBudgetManager({
  ...
  try {
    ...
    const metrics = await parallelRule.executeWithMaxParallelism(async (task) => {
      ...
      const result = await executeTaskWithRetry(
        ...
        recoveryManager,
        config,
        1,  // attemptNumber iniziale
        task.model as ModelTier  // model iniziale dal task
      );
    ...
    const totalDuration = performance.now() - executionStartTime;
