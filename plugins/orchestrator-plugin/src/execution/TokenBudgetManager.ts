...
export interface TokenThresholds {
  greenMax: number;      // 0-50% - Zona sicura
  yellowMax: number;     // 50-70% - Warning zone
  redMax: number;        // 70-85% - Decomposizione automatica
  criticalMax: number;   // 85%+ - Clear forzato
}
...
export type SplitStrategy =
  | 'by_subtask'        // Dividi in subtask logici
  | 'by_phase'          // Dividi per fasi (analisi, implementazione, test)
  | 'by_component'      // Dividi per componenti
  | 'by_priority'       // Dividi per priorità
  | 'sequential_chunks'; // Dividi in chunk sequenziali

export interface TokenBudgetConfig {
  maxTokensPerConversation: number;
  greenThresholdPercent: number;      // Default 50%
  yellowThresholdPercent: number;     // Default 70%
  redThresholdPercent: number;        // Default 85%
...
export class TokenBudgetManager extends EventEmitter {
  ...
  private lastZone: TokenZone = 'GREEN';
   ...
   */
  private calculateZone(usagePercent: number): TokenZone {
    if (usagePercent >= this.config.criticalThresholdPercent) return 'CRITICAL';
    if (usagePercent >= this.config.redThresholdPercent) return 'RED';
    if (usagePercent >= this.config.yellowThresholdPercent) return 'YELLOW';
    return 'GREEN';
  }
   ...
   */
  private checkZoneAndAct(budget: TokenBudget): {
    action: 'none' | 'warn' | 'decompose' | 'clear';
    message?: string;
  } {
   ...
   */
  decomposeTask(task: TaskForDecomposition, reason: DecompositionReason = 'token_limit_approaching'): DecompositionResult {
   ...
   */
  private determineSplitStrategy(task: TaskForDecomposition): SplitStrategy {
   ...
   */
  private generateSubTasks(task: TaskForDecomposition, strategy: SplitStrategy): TaskForDecomposition[] {
    const subTasks: TaskForDecomposition[] = [];
    const baseId = task.id;
    const targetTokensPerSubTask = Math.max(
      this.config.minSubTaskTokens,
      Math.floor(task.estimatedTokens / 3)
    );

    switch (strategy) {
      case 'by_phase':
        subTasks.push(
          this.createSubTask(baseId, 1, 'Analysis', task, 'Analyze requirements and design approach', targetTokensPerSubTask * 0.8),
          this.createSubTask(baseId, 2, 'Implementation', task, 'Implement the core functionality', targetTokensPerSubTask * 1.2),
          this.createSubTask(baseId, 3, 'Testing', task, 'Write tests and validate implementation', targetTokensPerSubTask * 0.8),
          this.createSubTask(baseId, 4, 'Integration', task, 'Integrate and finalize', targetTokensPerSubTask * 0.5)
        );
        break;
      ...
      case 'by_component':
        ...
        components.forEach((comp, i) => {
          subTasks.push(
            this.createSubTask(baseId, i + 1, comp, task, `Implement ${comp}`, targetTokensPerSubTask)
          );
        });
        break;
      ...
      case 'by_priority':
        subTasks.push(
          this.createSubTask(baseId, 1, 'Critical', task, 'Handle critical/core functionality first', targetTokensPerSubTask),
          this.createSubTask(baseId, 2, 'Important', task, 'Handle important but non-critical items', targetTokensPerSubTask),
          this.createSubTask(baseId, 3, 'Nice-to-have', task, 'Handle optional enhancements', targetTokensPerSubTask * 0.5)
        );
        break;
      ...
      case 'by_subtask':
        ...
        if (task.subTaskHints && task.subTaskHints.length > 0) {
          task.subTaskHints.forEach((hint, i) => {
            subTasks.push(
              this.createSubTask(baseId, i + 1, hint, task, hint, targetTokensPerSubTask)
            );
          });
        } else {
          ...
          for (let i = 0; i < Math.min(numSubTasks, 5); i++) {
            subTasks.push(
              this.createSubTask(baseId, i + 1, `Part ${i + 1}`, task,
                `Handle part ${i + 1} of ${task.description}`, targetTokensPerSubTask)
            );
          }
        }
        break;
      ...
      default:
        ...
        for (let i = 0; i < Math.min(numChunks, 4); i++) {
          subTasks.push(
            this.createSubTask(baseId, i + 1, `Chunk ${i + 1}/${numChunks}`, task,
              `Process chunk ${i + 1} of the task`, targetTokensPerSubTask)
          );
        }
        break;
    }
  ...
  private createSubTask(
    ...
    index: number,
    name: string,
    parentTask: TaskForDecomposition,
    description: string,
    estimatedTokens: number
  ): TaskForDecomposition {
    return {
      id: `${baseId}-sub${index}`,
   ...
   */
  private reduceComplexity(complexity: TaskForDecomposition['complexity']): TaskForDecomposition['complexity'] {
    switch (complexity) {
   ...
   */
  private updateStats(usagePercent: number): void {
   ...
   */
  private logBudgetStatus(budget: TokenBudget): void {
    const zoneColors: Record<TokenZone, string> = {
      'GREEN': '\x1b[32m',
      'YELLOW': '\x1b[33m',
      'RED': '\x1b[31m',
  ...
  private createProgressBar(percent: number): string {
    ...
    for (let i = 0; i < filled; i++) {
      if (i < width * 0.5) bar += '\x1b[32m█\x1b[0m';      // Green
      else if (i < width * 0.7) bar += '\x1b[33m█\x1b[0m'; // Yellow
      else if (i < width * 0.85) bar += '\x1b[31m█\x1b[0m'; // Red
      else bar += '\x1b[41m█\x1b[0m';                       // Critical
    }
    bar += '░'.repeat(empty);
...
export function getGlobalTokenBudgetManager(): TokenBudgetManager {
 ...
 */
export async function executeWithBudgetCheck<T>(
  taskDescription: string,
  estimatedTokens: number,
  executor: () => Promise<T>,
  onDecompose?: (result: DecompositionResult) => Promise<void>
): Promise<T | null> {
  ...
  if (!check.canExecute) {
    ...
    if (onDecompose) {
      const task: TaskForDecomposition = {
        id: `task-${Date.now()}`,
