...
import { SmartModelSelector, modelSelector, ModelSelectionResult } from './smart-model-selector';
import { AutoDocumenter, autoDocumenter, TaskDocumentation } from './auto-documenter';
import { AgentDiscovery, agentDiscovery, DiscoveryResult, AgentInfo } from './agent-discovery';
...
interface IntegratedTask {
    ...
    // Documentation (per Auto Documenter)
    workDone: string[];
...
interface ExecutorConfig {
    ...
    simulationDelayMs: { min: number; max: number };
    ...
    orchestratorModel: ModelType;  // SEMPRE Opus per orchestrator
...
export class IntegratedOrchestrator extends EventEmitter {
    ...
    private lastDashboardOutput: string = '';
     ...
     * Inizializza Agent Discovery
     */
    private async initializeAgentDiscovery(): Promise<void> {
        if (agentDiscovery.needsRefresh()) {
            await agentDiscovery.scan();
        }
    }
     ...
     */
    private resolveAgentFile(agentPath: string, taskDescription: string): {
        resolvedPath: string;
        wasFound: boolean;
        ...
        let result = agentDiscovery.findAgent(agentPath);

        // Se non trovato, cerca per task description
        if (!result.found) {
            result = agentDiscovery.findAgentForTask(taskDescription);
        }
    ...
    addRootTask(config: {
        description: string;
        agentExpertFile: string;
        model?: ModelType;  // Ora opzionale - usa smart selection
        priority?: PriorityLevel;
        canSpawnChildren?: boolean;
        maxChildren?: number;
    }): string {
        ...
        if (this.config.enableSmartModelSelection && !config.model) {
            const selection = modelSelector.selectModel(
                config.description,
                resolvedAgentFile,  // Usa agent risolto
    ...
    addTaskWithDependencies(config: {
        ...
        if (this.config.enableSmartModelSelection && !config.model) {
            const selection = modelSelector.selectModel(
                config.description,
                config.agentExpertFile,
                { depth: newDepth, isSubtask: config.dependsOn.length > 0 }
            );
            selectedModel = selection.model;
            modelReason = selection.reason;
            modelConfidence = selection.confidence;
     ...
     */
    private generateAndShowSessionReport(): void {
        const sessionMd = autoDocumenter.generateSessionMarkdown();
        const errorCatalogMd = autoDocumenter.generateErrorCatalogMarkdown();
        ...
        const report = autoDocumenter.generateSessionReport();
        console.log(`
📊 MODEL DISTRIBUTION
...
`);
     ...
     */
    private displayAgentDiscoverySummary(): void {
        const agents = agentDiscovery.getAllAgents();
        const plugins = agentDiscovery.getAllPlugins();
    ...
    getAgentDiscoveryReport(): string {
        return agentDiscovery.generateReport();
    }
    ...
    getAvailableAgents(): AgentInfo[] {
        return agentDiscovery.getAllAgents();
    }
    ...
    findAgentForTask(description: string): DiscoveryResult {
        return agentDiscovery.findAgentForTask(description);
    }
     ...
     */
    private getReadyTasks(): IntegratedTask[] {
        const ready: IntegratedTask[] = [];
        const availableSlots = this.config.maxConcurrent - this.runningTasks.size;

        if (availableSlots <= 0) return ready;
     ...
     */
    private async executeTaskAsync(task: IntegratedTask): Promise<void> {
        task.status = 'running';
     ...
     */
    private documentCompletedTask(task: IntegratedTask, status: 'SUCCESS' | 'FAILED' | 'PARTIAL'): void {
        const doc: TaskDocumentation = {
            taskId: task.id,
            taskPath: task.path,
        ...
        autoDocumenter.documentTask(doc);
    }

    /**
     * Invoca l'agent
     */
    private async invokeAgent(task: IntegratedTask): Promise<any> {
     ...
     */
    private generateMockSubtasks(parent: IntegratedTask): any[] {
     ...
     */
    private async spawnDynamicSubtasks(parent: IntegratedTask, configs: any[]): Promise<void> {
     ...
     */
    private unlockDependents(taskId: string): string[] {
        const task = this.tasks.get(taskId);
        if (!task) return [];
    ...
    getLevelStats(): LevelStats[] {
        const levelMap = new Map<number, IntegratedTask[]>();
...
`;
     ...
     * Mini progress bar
     */
    private renderMiniBar(progress: number, width: number): string {
     ...
     * Mostra report finale
     */
    private displayFinalReport(stats: GlobalStats): void {
        console.log('\n' + '═'.repeat(80));
