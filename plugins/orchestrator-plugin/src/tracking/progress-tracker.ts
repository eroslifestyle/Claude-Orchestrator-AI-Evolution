...
export class ProgressTracker extends EventEmitter {
  ...
  constructor(sessionId: string) {
    super();
    this.logger = new PluginLogger('ProgressTracker');
  ...
  private calculateMetrics(): ProgressMetrics {
    ...
    const maxPossibleParallel = Math.max(
      allTasks.filter(t => t.level === 1).length,
      allTasks.filter(t => t.level === 2).length,
      allTasks.filter(t => t.level === 3).length
    );
   ...
   */
  private calculateLevelMetrics(level: number): LevelMetrics {
